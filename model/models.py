...
class STGCN_ChebConv(nn.Module):
    ...
    def __init__(self, Kt, Ks, blocks, T, n_vertex, graph_conv_type, chebconv_filter_list, drop_rate):
        super(STGCN_ChebConv, self).__init__()
        self.st_block1 = layers.STConvBlock(Kt, Ks, n_vertex, blocks[0], graph_conv_type, chebconv_filter_list, drop_rate)
        self.st_block2 = layers.STConvBlock(Kt, Ks, n_vertex, blocks[1], graph_conv_type, chebconv_filter_list, drop_rate)
        Ko = T - (len(blocks) - 1) * 2 * (Kt - 1)
        if Ko > 1:
            self.output = layers.OutputBlock(blocks[-1], Ko, n_vertex, drop_rate)
        else:
...
class STGCN_GCNConv(nn.Module):
    ...
    def __init__(self, Kt, Ks, blocks, T, n_vertex, graph_conv_type, gcnconv_filter, drop_rate):
        super(STGCN_GCNConv, self).__init__()
        self.st_block1 = layers.STConvBlock(Kt, Ks, n_vertex, blocks[0], graph_conv_type, gcnconv_filter, drop_rate)
        self.st_block2 = layers.STConvBlock(Kt, Ks, n_vertex, blocks[1], graph_conv_type, gcnconv_filter, drop_rate)
        Ko = T - (len(blocks) - 1) * 2 * (Kt - 1)
        if Ko > 1:
            self.output = layers.OutputBlock(blocks[-1], Ko, n_vertex, drop_rate)
        else:
